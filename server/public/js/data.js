...
$(document).ready(function() {
    $(document).on("click", ".category", function() {
        ...
        $.getJSON(url, function (data) {
            $('body').append('<div class = "lot-info" id = "lot-info"><div class="lot-info-body"><button onclick="lotClose()" class="lot-info-close">Х</button><img class="lot-info-img" id="lot-img-'+data[current_id].id+'" src=".'+data[current_id].imgUrl+'" alt=""><div class="lot-info-text"><p class="lot-info-name fz36ffMain" id="lot-name-'+data[current_id].id+'">'+data[current_id].name+'</p><p class="lot-info-description" id="lot-description-'+data[current_id].id+'">'+data[current_id].description+'</p><p class="lot-info-price fz36ffMain" id="lot-price-'+data[current_id].id+'">'+data[current_id].price+' Р</p><button class="btn-add-to-cart fz36ffMain">В корзину</button></div></div></div>');
        });
    });
});
